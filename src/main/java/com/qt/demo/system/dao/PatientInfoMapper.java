...
public interface PatientInfoMapper {
    ...
    @Insert("insert into db_patient_info(patientID, name, age, height, weight, sex, birthPlace, illnessHistory, allergen, bloodType, smokeHistory, drinkHistory, sports, dmType, goutType, idCard, phoneNumber) " +
            "values (#{patientID}, #{name}, #{age}, #{height}, #{weight}, #{sex}, #{birthPlace}, #{illnessHistory}, #{allergen}, #{bloodType}, #{smokeHistory}, #{drinkHistory}," +
            "#{sports}, #{dmType}, #{goutType}, #{idCard}, #{phoneNumber})")
